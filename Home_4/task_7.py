...
def get_random_array() -> list[list[int]]:
    ...
    for i in range(PARTS):
        arr = [random.randint(MIN_NUM, MAX_NUM) for _ in range(1, int(ELEM / PARTS) + 1)]
        part_arr.append(arr)
    return part_arr
...
def get_sum_threads(arr: list[int]):
    ...
    for i in arr:
        sum_threads += i
    print(f'Summa of part: {sum_threads}, counts in {time.time() - start_time:.4f} seconds')
...
def get_sum_proc(arr: list[int], summ):
    for i in arr:
        with summ.get_lock():
            summ.value += i
    print(f'Summa of part: {summ.value}, counts in {time.time() - start_time:.4f} seconds')
